...
pub fn find_address_by_coordinates(lat: f64, lon: f64) -> Option<StaticAddressEntry> {
    ...
    let max_distance_km = 0.1; // 100 meters
    let mut closest: Option<(f64, StaticAddressEntry)> = None;

    for entry in data.values() {
        // Coordinates stored as [lat, lon]
...
mod tests {
    ...
    fn test_haversine_distance_same_point() {
        ...
        assert!(distance < 0.001); // Should be approximately 0
    }

    #[test]
    fn test_haversine_distance_known() {
        // Distance from Göteborg to Stockholm (approximately 470 km)
    ...
    fn test_haversine_distance_small() {
        ...
        assert!(distance > 0.1 && distance < 0.2); // ~100-200m
    }
}
