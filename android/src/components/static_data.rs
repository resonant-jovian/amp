...
//! }
//! ```
use amp_core::{parquet::read_db_parquet, structs::DB};
...
fn load_parking_data() -> HashMap<String, DB> {
    ...
    // Create a cursor from the embedded bytes - parquet crate can read from Cursor<&[u8]>
    let cursor = Cursor::new(PARQUET_BYTES);
    
    // Read parquet file - note: read_db_parquet expects File, so we need to adjust
    // For now, we'll need to use OutputData and convert to DB
    match read_db_parquet_from_bytes(PARQUET_BYTES) {
        Ok(records) => {
            ...
            for record in records {
                ...
                // Convert OutputData to DB using from_dag_tid
                if let Some(db) = DB::from_dag_tid(
                    ...
                    eprintln!(
                        "[StaticData] Failed to create DB from record: {} {} {}",
                        record.gata, record.gatunummer, dag
                    );
                }
            }
            eprintln!(
...
fn read_db_parquet_from_bytes(bytes: &[u8]) -> anyhow::Result<Vec<amp_core::structs::OutputData>> {
    ...
    while let Some(batch) = reader.next() {
        let batch = batch.map_err(|e| anyhow::anyhow!("Failed to read batch: {}", e))?;
        ...
        for i in 0..batch.num_rows() {
            let get_optional_string = |arr: &arrow::array::StringArray, idx: usize| -> Option<String> {
                if arr.is_null(idx) {
                    None
                } else {
