...
//! }
//! ```
use amp_core::{parquet::read_parquet, structs::DB};
...
/// `cargo run --release -- output --android`
const PARQUET_BYTES: &[u8] = include_bytes!("../../correlations.parquet");
...
///
/// # Returns
/// HashMap mapping address keys (format: "street_number_postalcode") to DB entries
///
/// # Panics
...
fn load_parking_data() -> HashMap<String, DB> {
    ...
    match read_parquet(PARQUET_BYTES) {
        Ok(records) => {
            let mut map = HashMap::new();
            for record in records {
                // Create key from address components for fast lookup
                let key = format!(
                    "{}_{}_{}_{}",
                    ...
                    record
                        ...
                        .unwrap_or_default(),
                    record.gatunummer.as_deref().unwrap_or(""),
                    record.dag,
                );
...
/// }
/// ```
pub fn get_address_data(street: &str, street_number: &str, postal_code: &str) -> Option<&DB> {
    let data = get_static_data();
    // Try to match all days (1-31) since we don't know which day the restriction is active
    for day in 1..=31 {
        let key = format!(
            "{}_{}_{}_{}",
            postal_code,
...
mod tests {
    ...
    fn test_get_all_postal_codes() {
        ...
        assert!(
            !codes.is_empty(),
            "Should have at least one postal code",
        );
        for i in 1..codes.len() {
            assert!(
                codes[i - 1] <= codes[i],
                "Postal codes should be sorted",
            );
        }
    }
    #[test]
    ...
    #[test]
    fn test_db_struct_fields() {
        let data = get_static_data();
        if let Some((_, db)) = data.iter().next() {
            assert!(db.start_time.is_some(), "DB should have start_time");
            assert!(db.end_time.is_some(), "DB should have end_time");
        }
    }
}
