...
# Generate Info.plist with all required keys for iOS native bridge
generate_ios_info_plist() {
    local PLIST_PATH="$1"
    local PLATFORM_NAME="${2:-iPhoneOS}"  # iPhoneOS or iPhoneSimulator
...
<dict>
	...
	<array>
		<string>PLATFORM_PLACEHOLDER</string>
	</array>
	...
	<string>DTPLATFORM_PLACEHOLDER</string>
	...
	<!-- GPS: Required for CLLocationManager -->
	<key>NSLocationWhenInUseUsageDescription</key>
	<string>AMP needs your location to find nearby parking restrictions.</string>

	<!-- Background tasks: Required for BGTaskScheduler dormant checks -->
	<key>BGTaskSchedulerPermittedIdentifiers</key>
	<array>
		<string>se.malmo.skaggbyran.amp.dormant</string>
	</array>

	<!-- Notifications -->
	<key>NSUserNotificationsUsageDescription</key>
	<string>AMP sends notifications about upcoming parking restrictions.</string>

	<!-- File sharing: makes Documents folder visible in Files app for import/export -->
...
PLIST
    ...
    if [ "$PLATFORM_NAME" = "iPhoneSimulator" ]; then
        sed -i '' 's/PLATFORM_PLACEHOLDER/iPhoneSimulator/' "$PLIST_PATH"
        sed -i '' 's/DTPLATFORM_PLACEHOLDER/iphonesimulator/' "$PLIST_PATH"
    else
        sed -i '' 's/PLATFORM_PLACEHOLDER/iPhoneOS/' "$PLIST_PATH"
        sed -i '' 's/DTPLATFORM_PLACEHOLDER/iphoneos/' "$PLIST_PATH"
    fi

    echo "  ✅ Info.plist generated with GPS, BGTaskScheduler, and notification keys"
}

# Patch an existing Info.plist (e.g. one generated by dx) to add required keys
patch_ios_info_plist() {
    local PLIST_PATH="$1"

    if [ ! -f "$PLIST_PATH" ]; then
        echo "  ⚠️  Info.plist not found at $PLIST_PATH, skipping patch"
        return 1
    fi

    echo "  🔧 Patching Info.plist with native bridge keys..."

    # Add NSLocationWhenInUseUsageDescription if missing
...
' "$PLIST_PATH"
    ...
    if ! grep -q "BGTaskSchedulerPermittedIdentifiers" "$PLIST_PATH"; then
        sed -i '' '/<\/dict>/i\
	<key>BGTaskSchedulerPermittedIdentifiers</key>\
	<array><string>se.malmo.skaggbyran.amp.dormant</string></array>
' "$PLIST_PATH"
        echo "    ✅ Added BGTaskSchedulerPermittedIdentifiers"
    else
        echo "    ✓ BGTaskSchedulerPermittedIdentifiers already present"
    fi

    # Add NSUserNotificationsUsageDescription if missing
    if ! grep -q "NSUserNotificationsUsageDescription" "$PLIST_PATH"; then
        sed -i '' '/<\/dict>/i\
	<key>NSUserNotificationsUsageDescription</key>\
	<string>AMP sends notifications about upcoming parking restrictions.</string>
' "$PLIST_PATH"
        echo "    ✅ Added NSUserNotificationsUsageDescription"
    else
        echo "    ✓ NSUserNotificationsUsageDescription already present"
    fi

    echo "  ✅ Info.plist patched"
}
# ========== END iOS SHARED HELPERS ==========

# ========== iOS DEVICE BUILD (dx build) ==========
build_ios() {
    echo ""
    echo "🍎 iOS device build (via dx)..."
    ...
    if [ -f "Dioxus.toml" ]; then
        ...
        echo "  ✓ Backed up Dioxus.toml"
    fi

    # Build with dx
    echo "  📦 Running dx build --release..."
    if ! dx build --release --verbose 2>&1; then
    ...
    echo "  ✅ dx build succeeded"

    # Find and patch the generated Info.plist
    echo ""
    echo "  🔧 Patching generated app bundle..."
    local DX_BUNDLE
    DX_BUNDLE="$(find "$REPO_ROOT/ios/dist" "$REPO_ROOT/target/dx" -name "*.app" -type d 2>/dev/null | head -n 1)"

    if [ -n "$DX_BUNDLE" ] && [ -d "$DX_BUNDLE" ]; then
        echo "  📍 Found app bundle: $DX_BUNDLE"
        local DX_PLIST="$DX_BUNDLE/Info.plist"

        if [ -f "$DX_PLIST" ]; then
            patch_ios_info_plist "$DX_PLIST"
        else
            echo "  ⚠️  No Info.plist in bundle, generating one..."
            generate_ios_info_plist "$DX_PLIST" "iPhoneOS"
        fi

        echo ""
        echo "  ✅ iOS device build complete!"
        echo "  📍 App bundle: $DX_BUNDLE"
    else
        echo "  ⚠️  Could not find .app bundle in dist/ or target/dx/"
        echo "  The build succeeded but bundle location is unknown."
    fi
    ...
    if [ -n "$DIOXUS_BACKUP" ] && [ -f "$DIOXUS_BACKUP" ]; then
        cp -- "$DIOXUS_BACKUP" "Dioxus.toml"
        rm -f -- "$DIOXUS_BACKUP"
        echo "  ✓ Restored Dioxus.toml"
    fi

    cd "$REPO_ROOT"
}
# ========== END iOS DEVICE BUILD ==========

# ========== iOS SIMULATOR BUILD (manual cargo + bundle) ==========
build_ios_sim() {
    echo ""
    echo "🍎 iOS simulator build..."

    # Detect host architecture for correct simulator target
    local ARCH
    ARCH="$(uname -m)"
    local TARGET
    if [ "$ARCH" = "arm64" ]; then
    ...
    local BUNDLE_DIR="$REPO_ROOT/ios/dist/AMP.app"
    ...
    # Workaround: dx build has a bug using iPhoneOS SDK instead of iPhoneSimulator
    # SDK for simulator targets, so we build with cargo directly.
    cd "$REPO_ROOT" || exit 1
    cargo build -p amp-ios --target "$TARGET" --release

    echo "  📁 Creating app bundle at $BUNDLE_DIR..."
    rm -rf "$BUNDLE_DIR"
    mkdir -p "$BUNDLE_DIR"
    cp "$REPO_ROOT/target/$TARGET/$PROFILE/amp" "$BUNDLE_DIR/amp"

    # Generate Info.plist with all required keys
    generate_ios_info_plist "$BUNDLE_DIR/Info.plist" "iPhoneSimulator"

    # Copy assets into bundle
    echo "  📄 Copying assets..."
    if [ -f "$REPO_ROOT/assets/style.css" ]; then
        cp "$REPO_ROOT/assets/style.css" "$BUNDLE_DIR/style.css"
        echo "    ✓ style.css"
    fi

    # iOS expects icon files named AppIcon*.png at the bundle root
    local ICON_SRC="$REPO_ROOT/assets/icon/icon-512.png"
    if [ -f "$ICON_SRC" ]; then
        # iOS uses specific sizes; 512px works as the base, iOS will scale down
        cp "$ICON_SRC" "$BUNDLE_DIR/AppIcon.png"
        cp "$ICON_SRC" "$BUNDLE_DIR/AppIcon60x60@2x.png"
        cp "$ICON_SRC" "$BUNDLE_DIR/AppIcon60x60@3x.png"
        cp "$ICON_SRC" "$BUNDLE_DIR/AppIcon76x76@2x.png"
        echo "    ✓ app icon"
    else
    ...
    echo ""
    echo "  ✅ iOS simulator build complete! App running."
    echo "  📍 Bundle: $BUNDLE_DIR"
    echo ""
    echo "  📝 Useful commands:"
    echo "     View logs:  xcrun simctl spawn booted log stream --predicate 'subsystem == \"$BUNDLE_ID\"'"
    echo "     Reinstall:  xcrun simctl install booted $BUNDLE_DIR"
    echo "     Relaunch:   xcrun simctl launch booted $BUNDLE_ID"
    echo "     Terminate:  xcrun simctl terminate booted $BUNDLE_ID"
...
case "$PLATFORM" in
    ...
    android)
        ...
        ;;
    *)
        echo "Unknown platform: $PLATFORM (valid: android, ios, ios-sim, both)"
        exit 1
        ;;
esac
