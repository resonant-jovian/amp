//! iOS-specific implementation for the Amp parking app.
//!
//! This crate provides the iOS UI layer built with Dioxus, along with
//! platform-specific utilities for storage, lifecycle management, and native
//! iOS integration through objc2.
//!
//! # Architecture
//!
//! The ios app follows a layered architecture:
//!
//! ```text
//! ┌─────────────────────────────────────┐
...
//! - [`lifecycle::LifecycleManager`]: ios activity lifecycle handling
...
//!
//! - [`ios_bridge`]: Native iOS integration via objc2
//! - [`ios_utils`]: iOS-specific file system access
//!
//! # Quick Start
//!
...
//! ```
//!
//! ## Configuring WebView (ios)
//!
//! ```no_run
//! use amp_ios::webview_config;
//!
//! # #[cfg(target_os = "ios")]
//! # {
...
//!
//! ```no_run
//! use amp_ios::storage::StorageManager;
//!
//! # async fn example() -> Result<(), Box<dyn std::error::Error>> {
...
//!
//! ```no_run
//! use amp_ios::components::matching::find_matching_parkering;
...
//! ## File System Access
//!
//! ios apps have restricted file system access. Use [`ios_utils`] to get
//! the correct paths:
//!
//! ```no_run
//! # #[cfg(target_os = "ios")]
//! # {
//! use amp_ios::get_ios_files_dir;
//!
...
//!
//! ## Platform Features
//! - ios lifecycle-aware background processing
...
//! - [`LifecycleManager`]: ios lifecycle management
...
#[cfg(target_os = "ios")]
pub use ios_utils::{get_ios_files_dir, init_ios_storage};
