...
//! Extracted from the original main.rs to support both CLI and TUI modes

use amp_core::api::{api, api_miljo_only};
...
pub enum Commands {
    ...
    /// Interactive Ratatui TUI for correlation and testing
    Tui,
...
pub fn run_correlation(
    ...
    let (addresses, miljodata, parkering): (Vec<AdressClean>, Vec<MiljoeDataClean>, Vec<MiljoeDataClean>) = api()?;
...
/// Run benchmark
pub fn run_benchmark(sample_size: usize, cutoff: f64) -> Result<(), Box<dyn std::error::Error>> {
    // Load data
    let pb = ProgressBar::new_spinner();
...
pub async fn check_updates(checksum_file: &str) -> Result<(), Box<dyn std::error::Error>> {
